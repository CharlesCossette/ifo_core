...
class TwistPublisherNode(object):
    def __init__(self):
        super(TwistPublisherNode, self).__init__()
        rospy.init_node("twist_ros")
        ...
        for model in model_names:
            ...
            if model != "ground_plane":
                self.pubs[model] = rospy.Publisher(
                    "twist_node/"+ model + "/twist", TwistStamped, queue_size=1
                )
    ...
    def publish_twists(self):
        states = self.model_states
        timestamp = self.timestamp
        twists = dict(zip(states.name, states.twist))

        for model_name in twists:
            ...
            if model_name != "ground_plane":
                ...
                if not do_gap_test:
                    ...
                    if not do_zero_test:
                        # twist message gets initialized to zero by default
                        twist = twists[model_name]
                        twist_msg.twist.linear = twist.linear
                        twist_msg.twist.angular = twist.angular
